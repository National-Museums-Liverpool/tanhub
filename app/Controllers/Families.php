<?php

namespace App\Controllers;

use App\Models\FamilyModel;
use CodeIgniter\Exceptions\PageNotFoundException;

/**
 * Admin read-only views for families.
 */
class Families extends BaseController
{
    /**
     * Display a paginated, sortable list of families with related taxa counts.
     */
    public function index(): string
    {
        $sort = strtolower((string) $this->request->getGet('sort'));
        $direction = strtolower((string) $this->request->getGet('direction'));

        $allowedSortColumns = ['id', 'taxon_identifier', 'scientific_name', 'vernacular_name'];

        if (! in_array($sort, $allowedSortColumns, true)) {
            $sort = 'scientific_name';
        }

        if (! in_array($direction, ['asc', 'desc'], true)) {
            $direction = 'asc';
        }

        /** @var FamilyModel $model */
        $model = model(FamilyModel::class);
        $families = $model->orderBy($sort, $direction)->paginate(20);

        $countsByFamilyId = $this->getTaxaCountsByForeignKey('family_id', array_column($families, 'id'));

        foreach ($families as &$family) {
            $family['taxa_count'] = $countsByFamilyId[(int) $family['id']] ?? 0;
        }
        unset($family);

        return $this->renderPage('families/index', [
            'pageTitle' => 'Families',
            'metaDescription' => 'Families list.',
            'bodyClass' => 'app-shell',
            'families' => $families,
            'pager' => $model->pager,
            'sort' => $sort,
            'direction' => $direction,
        ]);
    }

    /**
     * Show read-only details for a single family.
     */
    public function show(int $id): string
    {
        /** @var FamilyModel $model */
        $model = model(FamilyModel::class);
        $family = $model->find($id);

        if ($family === null) {
            throw PageNotFoundException::forPageNotFound();
        }

        $taxaCount = $this->getTaxaCountForForeignKey('family_id', $id);

        return $this->renderPage('families/show', [
            'pageTitle' => 'Family details',
            'metaDescription' => 'Read-only family details.',
            'bodyClass' => 'app-shell',
            'family' => $family,
            'taxaCount' => $taxaCount,
        ]);
    }

    /**
     * Return taxa counts keyed by a foreign key for the provided IDs.
     *
     * @param array<int, int|string> $ids
     * @return array<int, int>
     */
    private function getTaxaCountsByForeignKey(string $foreignKey, array $ids): array
    {
        if ($ids === []) {
            return [];
        }

        $rows = db_connect()
            ->table('taxa')
            ->select($foreignKey . ' AS related_id, COUNT(*) AS taxa_count')
            ->whereIn($foreignKey, $ids)
            ->where('deleted_at', null)
            ->groupBy($foreignKey)
            ->get()
            ->getResultArray();

        $counts = [];

        foreach ($rows as $row) {
            $counts[(int) $row['related_id']] = (int) $row['taxa_count'];
        }

        return $counts;
    }

    /**
     * Return the number of taxa for a single foreign key value.
     */
    private function getTaxaCountForForeignKey(string $foreignKey, int $id): int
    {
        return (int) db_connect()
            ->table('taxa')
            ->where($foreignKey, $id)
            ->where('deleted_at', null)
            ->countAllResults();
    }
}
