...
```cron
*/2 * * * * /usr/bin/php /var/www/tanhub/spark import:auto >> /var/www/tanhub/writable/import-logs/import-$(date +\%F).log 2>&1
```

Replace `/var/www/tanhub` with the absolute application path and use the PHP
binary configured for the deployment. Every CLI and admin import holds a non-blocking file lock at
`writable/import.lock` for the process lifetime and also uses a MySQL advisory lock for cross-host
coordination. The file lock continues to protect a host if MySQL closes an idle advisory-lock
connection during a long import, so no external `flock` utility is required. The web-server and
CLI users that run imports must have permission to create and write the same lock file. Choose an
interval that allows a normal batch to finish and respects the rate limits of the configured
